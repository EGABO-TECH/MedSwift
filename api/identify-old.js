...
function extractClinicalStructure(text) {
  // Parse clinical text to extract: chief complaint, findings, diagnosis, medications, plan
  ...
  const complaintKeywords = /(?:chief complaint|cc:|patient presents|complains of|c\/c|chief)/i;
  const findingKeywords = /(?:exam|physical exam|pe|vital|bmi|bp|hr|o2|finding|observed|noted|shows)/i;
  const diagKeywords = /(?:diagnosis|dx:|assessment|impression|rule out|ddx)/i;
  const medKeywords = /(?:medication|drug|rx|prescribed|taking|given|mg|tablet|capsule|dose)/i;
  const planKeywords = /(?:plan|recommendation|advise|follow.?up|return|recheck|monitor|continue|stop)/i;

  for (const line of lines) {
    if (complaintKeywords.test(line)) clinical.complaints.push(line.replace(complaintKeywords, '').trim());
    else if (diagKeywords.test(line)) clinical.diagnoses.push(line.replace(diagKeywords, '').trim());
    else if (medKeywords.test(line)) clinical.medications.push(line);
    else if (planKeywords.test(line)) clinical.plan.push(line.replace(planKeywords, '').trim());
    else if (findingKeywords.test(line)) clinical.findings.push(line.replace(findingKeywords, '').trim());
    else if (line.length > 10) clinical.findings.push(line);
  }
...
async function createOfflineFallbackResult(mode, extractedText, medications = []) {
  ...
  if (mode === 'report') {
    ...
    const finalComplaints = clinical.complaints.slice(0, 2).filter(Boolean);
    const finalFindings = clinical.findings.slice(0, 3).filter(Boolean);
    const finalDiagnoses = clinical.diagnoses.slice(0, 2).filter(Boolean);
    const finalPlan = clinical.plan.slice(0, 2).filter(Boolean);
    
    const title = finalDiagnoses.length > 0 
      ? finalDiagnoses[0].slice(0, 40) 
      : (finalComplaints.length > 0 ? 'Evaluation of ' + finalComplaints[0].slice(0, 30) : 'Clinical Assessment');
    
    const summary = [
      ...(finalComplaints.length > 0 ? [`Patient with ${finalComplaints.join(', ')}.`] : []),
      ...(finalFindings.length > 0 ? [`Examination reveals ${finalFindings.join(', ')}.`] : []),
      ...(finalDiagnoses.length > 0 ? [`Assessment: ${finalDiagnoses.join('; ')}.`] : [])
    ].join(' ').slice(0, 200);
    
    return {
      title: title || 'Clinical Assessment',
      summary: summary || 'Patient evaluation completed. Review clinical notes for full documentation.',
      keyFindings: finalFindings.length > 0 ? finalFindings : ['Clinical examination performed'],
      recommendations: finalPlan.length > 0 ? finalPlan : ['Continue current management', 'Follow-up as needed'],
      followUp: 'Reassess as clinically indicated based on patient response.',
      confidenceScore: 58,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    let prompt = '';
    if (mode === 'report') {
      prompt = `You are a clinical documentation specialist. Extract and structure the clinical information from this medical note into a concise, actionable summary.

**CRITICAL RULES:**
1. Extract ONLY facts stated in the document—never invent diagnoses, findings, tests, or medications.
2. Structure output by: Chief Complaint → Findings → Assessment → Plan
3. Ignore administrative text, form fields, and boilerplate unless it contains clinical information.
4. If a section is missing, leave it as "Not stated in the provided note."
5. Use plain clinical language. Keep each statement short and specific.
6. Return ONLY valid JSON—no markdown code blocks, no prose, no extra text.
...
{
  "title": "Brief clinical summary title (3-8 words, e.g., 'Hypertension Management' or 'Acute Upper Respiratory Infection')",
  "summary": "One to two sentences stating the patient presentation and main assessment. Must be clinically coherent and actionable. Example: 'Patient with new-onset hypertension. BP today 165/102. Started on amlodipine 5mg daily.'",
  "keyFindings": [
    "Most critical clinical fact (e.g., 'SBP 165 mmHg, elevated from baseline')",
    "Second finding (e.g., 'No acute symptoms reported')",
    "Third finding if present (e.g., 'Heart rate regular, lungs clear')"
  ],
  "recommendations": [
    "Specific, actionable next step (e.g., 'Start amlodipine 5mg daily')",
    "Follow-up action (e.g., 'Recheck BP in 2 weeks')"
  ],
  "followUp": "Single sentence describing timing and type of follow-up (e.g., 'Return to clinic in 2 weeks for BP recheck and medication adjustment.')",
  "confidenceScore": 85,
  "sourceContext": "Type of note (e.g., 'Outpatient Visit Note' or 'Physician Encounter')"
}
`;
